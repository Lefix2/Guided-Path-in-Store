...
store *my_test_store_new()
{
	...
	store_add_new_section(sttest, id++, t_wall, 0, 0, 1, magsizey);
	store_add_new_section(sttest, id++, t_wall, magsizex - 1, 0, 1, magsizey);
	store_add_new_section(sttest, id++, t_wall, 1, 0, magsizex - 2, 1);
	store_add_new_section(sttest, id++, t_wall, 1, magsizey - 2, 2, 2);
	store_add_new_section(sttest, id++, t_checkout, 3, magsizey - 3, checkoutsize, 3);
	store_add_new_section(sttest, id++, t_wall, checkoutsize + 3, magsizey - 2, magsizex - entrancesize - 20, 2);
	store_add_new_section(sttest, id++, t_entrance, magsizex - entrancesize - 7, magsizey - 3, entrancesize, 3);
	store_add_new_section(sttest, id++, t_wall, magsizex - entrancesize , magsizey - 2, 6, 2);

	for (x = 2; x < magsizex / 3 - 4; x += 5)
	{
		for (y = 2; y < 2 * magsizey / 3 - 3; y += 6)
		{
			store_add_new_section(sttest, id++, t_section, x, y, 3, 4);
		}
		store_add_new_section(sttest, id++, t_section, x, 2 * magsizey / 3, 2, ysectionlenght);
	}

	for (x = magsizex / 3 + 1; x < 2*magsizex / 3 - 4; x += 3)
	{
		store_add_new_section(sttest, id++, t_section, x, 1, 2, ysectionlenght);
		store_add_new_section(sttest, id++, t_promo, x, magsizey / 3 + 1, 2, ysectionlenght);
		store_add_new_section(sttest, id++, t_section, x, 2*magsizey / 3 + 1, 2, ysectionlenght);
	}

	for (y = 3; y < magsizey-5; y += 3)
	{
		store_add_new_section(sttest, id++, t_section, xsection3start, y, xsectionlenght, 2);
	}


	store_add_new_item(sttest, 0, legumes_vert, "haricot");
	store_add_new_item(sttest, 1, alcool, "desperado");
	store_add_new_item(sttest, 2, fromage, "gruyere");
	store_add_new_item(sttest, 3, pain, "brioche");
	store_add_new_item(sttest, 4, alcool, "vin");
	store_add_new_item(sttest, 5, legumes_vert, "petits poids");
	store_add_new_item(sttest, 6, fromage, "gouda");
	store_add_new_item(sttest, 7, pain, "biscottes");
	store_add_new_item(sttest, 8, legumes_vert, "asperges");
